<script lang="ts">
	import { shareLinkState, getAllShareLinks } from "@/data/share-link/store.svelte";
			...
			{#if platform === "Twitter / X"}
				<div class="mt-6">
					<label for="twitterTextarea">
						<small>Enter template text (optional – link already added)</small>
						<textarea
							id="twitterTextarea"
							maxlength="280"
							rows="3"
							...
							aria-label="Twitter template text"></textarea>
					</label>
					<small>{shareLinkState.TwitterParameters.text.length} characters</small>
					<div class="mt-6">
						<label for="svelte-tags-input">
							<small>Type any hashtags (optional) – Enter to add a hashtag</small>
						</label>
						<Tags
			...
			{#if platform === "WhatsApp"}
				<label class="mt-6">
					...
					<textarea
						...
						aria-label="WhatsApp template text"></textarea>
				</label>
			{/if}
				...
				<div class="mt-6">
					...
					<label>
						...
						<textarea
							...
							aria-label="Email body text"></textarea>
					</label>
				</div>
			{/if}
			...
			{#if platform === "Blue Sky"}
				<label class="mt-6">
					...
					<textarea
						...
						aria-label="Blue Sky template text"></textarea>
				</label>
			{/if}
