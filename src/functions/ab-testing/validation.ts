import { z } from "zod";
import type { TestVariation, ABTestInput, TwoProportionTestData, MultiVariationTestData } from "../../types/ab-testing";

/**
 * Zod schema for validating a single test variation
 * Provides clear, actionable error messages for both novices and experts
 */
export const testVariationSchema = z.object({
	name: z.string()
	...
	(data) => data.conversions <= data.visitors,
	{
...
export const abTestInputSchema = z.object({
	...
	variations: z.array(testVariationSchema)
	...
	confidenceLevel: z.number()
		.min(0.80, "We recommend at least 80% confidence - anything lower isn't reliable")
		.max(0.99, "99% is the highest confidence level we support")
		.refine(
			(level) => [0.80, 0.85, 0.90, 0.95, 0.99].includes(level),
			{
 ...
 */
export const twoProportionTestDataSchema = z.object({
	n1: z.number()
 ...
 */
export function validateABTestInput(input: unknown): 
...
export function validateTwoProportionTestData(input: unknown):
...
export function validateMultiVariationTestData(input: unknown):
...
export function validateStatisticalRequirements(data: ABTestInput): string[] {
	...
	allVariations.forEach((variation, index) => {
		...
		if (variation.visitors < 100) {
			warnings.push(`${label} has only ${variation.visitors} visitors. For reliable results, try to get at least 100 visitors per variation before drawing conclusions.`);
		}
		
		// Low conversion warning with context
		if (variation.conversions < 5) {
			warnings.push(`${label} has only ${variation.conversions} conversions. With fewer than 5 conversions, the results might change significantly with just a few more data points.`);
		}
		...
		if (conversionRate < 0.005) {
			warnings.push(`${label} has a ${(conversionRate * 100).toFixed(2)}% conversion rate. Very low rates need much larger sample sizes - consider running your test longer or checking if your tracking is working correctly.`);
		}
		
		// Zero conversions handling
		if (variation.conversions === 0) {
			warnings.push(`${label} has zero conversions. The statistical test can still run, but you'll need at least one conversion to calculate meaningful improvement percentages.`);
		}
	});
	...
	const identicalRates = data.variations.filter(v => 
		Math.abs((v.conversions / v.visitors) - controlRate) < 0.0001
	);
	
	if (identicalRates.length > 0) {
		warnings.push("Some variations have identical conversion rates. This is fine early in a test, but if rates stay identical with large sample sizes, there may be no real difference to detect.");
	}
	...
	if (maxVisitors > minVisitors * 3) {
		warnings.push("Your sample sizes are quite unbalanced. While the test will still work, more balanced groups usually give clearer results.");
	}
