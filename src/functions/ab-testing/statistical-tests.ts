...
function groupVariationsByPerformance(
	...
	for (let i = 0; i < sortedVariations.length; i++) {
		...
		if (!placedInTier) {
			...
			const tierLabel =
				tierNumber === 1
					? `${confidencePrefix}Highest Performers`
					: tierNumber === 2
						? `${confidencePrefix}Middle Performers`
						: tierNumber === 3
							? `${confidencePrefix}Lower Performers`
...
function generateBusinessInsights(
	...
	if (performanceGroups.length > 1) {
		...
		if (topTier.variations.length === 1 && bottomTier.variations.length === 1) {
			...
			const topNames = new Intl.ListFormat("en", { style: "long", type: "conjunction" }).format(
				topTier.variations.map((v) => `<em>${v.name}</em>`)
			);
			const avgTopRate =
				topTier.variations.reduce((sum, v) => sum + v.conversionRate, 0) /
				topTier.variations.length;
